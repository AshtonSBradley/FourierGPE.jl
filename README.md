...
using FourierGPE
```
Note that `FourierGPE` uses `FFTW#master` built with `MKL`. This _should_ avoid conflicts with `FFTW` releases, and avoid breaking packages like `FastTransforms` that require non `MKL` `FFT`.
